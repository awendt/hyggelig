namespace :app do
  ...
  task :setup do
    ...
    system 'rake --silent secret:generate_file' unless File.exists?("#{RAILS_ROOT}/config/session_key_secret")
    puts 'Copying database.yml...'
    system 'cp config/template.database.yml config/database.yml' unless File.exists?("#{RAILS_ROOT}/config/database.yml")
